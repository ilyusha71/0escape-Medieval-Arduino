/******************************************
  Title: Magic Altar - Heart
  Studio: Wakaka KocmocA & 0escape
  Author: By iLYuSha Wakaka KocmocA
  2016/04/05

  X6 (234745CA: 35,71,69,202)
  X8 (A34746CA: 163,71,70,202)
*******************************************/
/******************************************
...
const int d2 = 202;
/* Output */
const int destruction = 2;
const int destructionLED = 3;
const int redemption = 4;
const int redemptionLED = 5;
/* Variable */
int escape = 0;
...
void setup() {
    ...
    digitalWrite(destructionLED, LOW);
    digitalWrite(redemptionLED, LOW);
    Serial.println("Magic Altar - Heart 2016/03/12 iLYuSha");
}
...
void loop()
{ 
  if(mfrc522.uid.uidByte[0] == a1 && mfrc522.uid.uidByte[1] == b1 && mfrc522.uid.uidByte[2] == c1 && mfrc522.uid.uidByte[3] == d1)
  {
    digitalWrite(destruction, HIGH);
    digitalWrite(destructionLED, HIGH);
  }
  else if(mfrc522.uid.uidByte[0] == a2 && mfrc522.uid.uidByte[1] == b2 && mfrc522.uid.uidByte[2] == c2 && mfrc522.uid.uidByte[3] == d2)
  {
    digitalWrite(redemption, HIGH);
    digitalWrite(redemptionLED, HIGH);
  }
  else
  {
    if(escape > 1)
