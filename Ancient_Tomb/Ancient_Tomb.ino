...
int valueTombGate = 0;
/* Output */
const int emLockDestruction = 4;
const int emLockRedemption = 5;
const int emLockTombGate = 6;
const int soundTraitor = 7;
const int soundRedeemer = 8;
/* Variable */
boolean passTraitor = false;
boolean passRedeemer = false;
boolean unlockTomb = false;
...
void setup() {
  ...
  pinMode(soundTraitor,OUTPUT);
  pinMode(soundRedeemer,OUTPUT);
  digitalWrite(emLockDestruction, HIGH);
  digitalWrite(emLockRedemption, HIGH);
  digitalWrite(emLockTombGate, HIGH);
  digitalWrite(soundTraitor, HIGH);
  digitalWrite(soundRedeemer, HIGH);
  Serial.println("Ancient Tomb 2016/04/05 iLYuSha Wakaka KocmocA");
}
...
void loop()
{
  ...
  valueTombGate = digitalRead(reedSwitchTombGate);
  
  /* Traitor Mission */
  if(valueIR == 1 && valueTombGate == 1 && !passTraitor)
  {
    digitalWrite(emLockDestruction, LOW);
    digitalWrite(soundTraitor, LOW);
    passTraitor = true;        
  }
  
  /* Redeemer Mission*/
  if(mfrc522.uid.uidByte[0] == a1 && mfrc522.uid.uidByte[1] == b1 && mfrc522.uid.uidByte[2] == c1 && mfrc522.uid.uidByte[3] == d1 && !passRedeemer)
  {
    digitalWrite(emLockRedemption, LOW);
    digitalWrite(soundRedeemer, LOW);
    passRedeemer = true;
  }
  else if(mfrc522.uid.uidByte[0] == a2 && mfrc522.uid.uidByte[1] == b2 && mfrc522.uid.uidByte[2] == c2 && mfrc522.uid.uidByte[3] == d2 && !unlockTomb)
  {
